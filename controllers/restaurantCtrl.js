...
const restaurantCtrl = {
    ...
    postInfo: async (req,res)=>{
        try {
            ...
            const {id, name, email, category, description, contact, mainPhoto, menus, bestSeller, location} =  req.body
            const newRestaurant = new restaurants({id, name, email, category, description, contact, mainPhoto, menus, bestSeller, location})
    ...
    updateInfo: async (req,res)=>{
        const {id, name, email, photos, category, description, contact, mainPhoto, bestseller, location} =  req.body
        if(!id) return res.status(400).json({msg: "Please provide the id to process the update!"})
        await restaurants.findOneAndUpdate({id: id}, {$set : {name: name, email: email, photos: photos, category: category, description: description, contact: contact, mainPhoto: mainPhoto, bestSeller: bestseller,location: location}}, (err,result)=>{
    ...
    getAll : async (req,res)=>{
        try {
            const allRestaurants = await restaurants.find({}, {_id:0, id:1, name:1, location:1,mainPhoto:1}, (err, result)=>{
    ...
    getSimilar: async(req,res)=>{
        try {
            const {id} = req.body
            const restaurant = await restaurants.findOne({id: id}, {_id:0, category:1})
            ...
            const similar = await restaurants.find({
                ...
                id : {$ne: id},
            }, {_id:0, id:1, name:1, mainPhoto:1, location:1}, (err, result)=>{
                if(err) return res.status(400).json({msg: err.msg})
                return result;
            })
            res.json(similar)
