...
class dashboard extends Component {
    ...
    async onSortingPrice(e){
        let option = e;
        if(option === "highToLow"){
            let sortedList = [...this.state.suggestions].sort((a,b) => b.menu.price-a.menu.price);
            await this.setState(() => ({suggestions: sortedList}));
            sortedList.map((cata)=>{
                console.log(cata.menu.price);
            })
        }else{
            let sortedList = [...this.state.suggestions].sort((a,b) => a.menu.price-b.menu.price);
            await this.setState(() => ({suggestions: sortedList}));
        }
    ...
    renderCatagories(){
        const MenuCataSection = () =>(
            ...
            <div className="selection">
                <div className="selectionChild1" >
                    <select onChange={(e)=>{this.onSortingPrice(e.target.value)}}>     
        ...
        const RestaurantCataSection = () => (
            ...
            <div className="selection">
                <div className="selectionChild1" >
                    <select onChange={(e)=>{this.onSorting(e.target.value)}}>  
                        <option value="" selected disabled hidden >Sort By</option>                  
            ...
            <div>
                <div className="search-section">
                    <div className="search-select1" >
                        <select value={this.searchTerm} className="search-change-section" onChange={(e) => this.changeSearchTerm(e)}>                       
